...
fn write_git_exclude(repo: &Path, content: &str) {
    let exclude_path = git(repo, &["rev-parse", "--git-path", "info/exclude"]);
    let exclude_path = repo.join(exclude_path);
...
fn resolve_use_remote_avoids_existing_ignored_target_path() {
    ...
    write_git_exclude(&a.skills, "beta/\n");
...
fn remote_deletion_leaves_local_ignored_files_untouched() {
    ...
    a.commit("local");
    write_git_exclude(&a.skills, "debug.log\n");
